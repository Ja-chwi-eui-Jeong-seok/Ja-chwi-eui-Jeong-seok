...
class Guide3 extends StatelessWidget {
  ...
  const Guide3({super.key, required this.onNext});
  ...
  Widget build(BuildContext context) {
    ...
    // 설명 텍스트 문자열
    const guideText = "집먼지에게 말을 걸어보세요! \nAI기반으로 때론 말동무, \n때로는 조언자가 되어줄 거에요!";
    ...
    final textTop = size.height * 0.34;

    // 화살표 끝점 = 텍스트 하단 중앙
    const arrowMargin = 60.0;
...
class _CurvedArrowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final arrowLength = 15.0; // 화살표 길이
    final arrowAngle = pi / 4; // 45도 각도로 벌어지게
    final direction = pi / 0.97;
